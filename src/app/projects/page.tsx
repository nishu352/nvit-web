      ...
      <main className="flex-1 py-14 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full space-y-12">
        ...
        <div className="space-y-8">
          {FEATURED_PROJECTS.map((proj, idx) => (
            <div
              key={idx}
              className="p-7 sm:p-9 rounded-3xl glass-card-apple space-y-6 hover:border-blue-500/40 transition-all shadow-sm"
            >
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="space-y-2 max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2.5">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-zinc-100 dark:bg-white/10 text-zinc-700 dark:text-zinc-300 border border-zinc-200/80 dark:border-white/10">
                      {proj.badge}
                    </span>
                  </div>
                ...
                <div className="flex flex-wrap md:flex-col gap-2.5 shrink-0 pt-2 md:pt-0">
                  {proj.liveHref && (
                    <Link href={proj.liveHref}>
                      <button className="px-5 py-2.5 rounded-xl bg-zinc-900 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-100 text-white dark:text-zinc-950 font-bold text-xs cursor-pointer shadow-sm transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
                  ...
                  {proj.caseStudyHref && (
                    <Link href={proj.caseStudyHref}>
                      <button className="px-5 py-2.5 rounded-xl bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 font-bold text-xs border border-zinc-200/80 dark:border-white/10 cursor-pointer transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
              ...
              <div className="pt-4 border-t border-zinc-100 dark:border-white/10 space-y-2.5">
                <span className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 block font-mono">
              ...
              <div className="flex flex-wrap items-center gap-1.5 pt-1">
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mr-1 font-mono">
                  Stack:
                </span>
                {proj.tech.map((t, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-white/5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400"
                  >
                    {t}
                  </span>
                ))}
        ...
        <section className="p-8 sm:p-12 rounded-3xl glass-card-apple flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
